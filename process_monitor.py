...
class ProcessMonitor:
    ...
    def __init__(self, target_script="main.py", check_interval=60, auto_restart=True,
                 max_memory_percent=90, max_cpu_percent=95):
        """
        초기화 함수
    ...
    def write_pid_file(self, pid):
        ...
        try:
            with open(self.pid_file, 'w') as f:
                f.write(str(pid))
            logger.info(f"PID {pid}를 파일에 저장했습니다.")
        except Exception as e:
            logger.error(f"PID 파일 저장 중 오류 발생: {e}")
    ...
    def start_target_process(self):
        ...
        try:
            ...
            log_file = f"{os.path.splitext(self.target_script)[0]}.log"
            with open(log_file, 'a') as f:
                logger.info(f"대상 스크립트 시작: {self.script_path} (로그: {log_file})")
    ...
    def run_monitor(self):
        ...
        while self.running:
            try:
                ...
                # 3. PID 파일에서 프로세스 ID 확인
                target_pid = self.check_pid_file()
                
                # 4. PID 파일이 없으면 실행 중인 프로세스 찾기
                ...
                if not process_running and self.auto_restart:
                    ...
                    restart_count += 1
                    logger.info(f"프로세스가 실행 중이지 않습니다. 재시작합니다. (시도 #{restart_count})")
                    ...
                    # 프로세스 시작
                    new_pid = self.start_target_process()
                    if new_pid:
                        self.write_pid_file(new_pid)
                        last_restart_time = datetime.now()
                        logger.info(f"프로세스 재시작 성공: PID={new_pid}")
                    else:
                        logger.error("프로세스 재시작 실패")
